...
def btn_quet_xe_ra(listCanvasVao: utils.ListCanvas, listCanvasRa: utils.ListCanvas, labels_info: utils.LabelGroup):
    # reset all image
    reset_canvas(listCanvasVao=listCanvasVao, listCanvasRa=listCanvasRa, labels_info=labels_info)

    # thực hiện quá trình xử lí xe ra
    bien_so_quet, image_duoi_xe, bsx_duoi, image_dau_xe, bsx_dau, image_logo, data_xe_vao, sameImage = xeRa.run_license_scan(label_thong_bao, window, labels_info.bien_so_xe)
...
btn2 = tk.Button(window, text="Quét xe ô tô ra", font=("Arial", 16), command=partial(btn_quet_xe_ra, listCanvasVao, listCanvasRa, labels), width=20, height=5, bg='yellow')
btn2.grid(row=2, column=2, rowspan=2, padx=10, pady=10)

btnAuto = tk.Button(window, text=f"Auto: {'Bật' if autoData else 'Tắt'}", command=click_auto, width=20, height=3, bg='orange')
